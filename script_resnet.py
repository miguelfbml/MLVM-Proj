...
def main():
    ...
    model = ResNet50Embedding(cfg.EMBEDDING_DIM).to(cfg.DEVICE)
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        if score > best_score:
            best_score = score
            early_stop_counter = 0
            torch.save(model.state_dict(), "best_logo_retrieval.pth")
    ...
    plt.legend(); plt.savefig('train_loss.png', dpi=300)

    # Score
    plt.figure()
    plt.plot(epochs, history['score'], 'g-', label='Score', linewidth=2)
    plt.xlabel('Epoch'); plt.ylabel('Score'); plt.title('Combined Score'); plt.grid(True)
    plt.legend(); plt.savefig('score.png', dpi=300)
    ...
    plt.xlabel('Epoch'); plt.ylabel('Precision'); plt.title('Precision@K'); plt.grid(True)
    plt.legend(); plt.savefig('precision.png', dpi=300)
    ...
    plt.xlabel('Epoch'); plt.ylabel('Recall'); plt.title('Recall@K'); plt.grid(True)
    plt.legend(); plt.savefig('recall.png', dpi=300)
